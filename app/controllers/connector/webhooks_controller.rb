class Connector::WebhooksController < ActionController::API
  ...
  def webhook
    ...
    when "activity.created"
      return unless @tenant.receive_customer_activities? || data[:activity_type].in?([ "agent_portal_and_backoffice_comment", "agent_backoffice_comment" ])
  ...
  def webhook_params
    params.permit(:type, :timestamp, data: [ :subject_id, :issue_id, :activity_id, :activity_type ])
  end

  def data
    params.require(:data).permit(:subject_id, :issue_id, :activity_id, :activity_type)
  end
