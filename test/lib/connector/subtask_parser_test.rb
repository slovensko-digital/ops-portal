...
class Connector::SubtaskParserTest < ActiveSupport::TestCase
  ...
  test "parses valid subtasks correctly" do
    parser = Connector::SubtaskParser.new(@sample_article_body)
  ...
  test "returns empty array when no subtask tag present" do
    ...
    parser = Connector::SubtaskParser.new(article_body)
    subtasks = parser.parse_subtasks
  ...
  test "returns empty array when subtask tag present but no valid subtask lines" do
    article_body = <<~BODY
      ...
      [[poduloha]]<br>Regular text without subtask format<br>Another line
    BODY
  ...
  test "handles multiple subtask tags (uses last one)" do
    article_body = <<~BODY
      ...
      [[poduloha]]<br><a href="http://vm-home/#user/profile/6" data-mention-user-id="6" rel="nofollow noreferrer noopener" title="http://vm-home/#user/profile/6">Old Agent</a> - Old subtask - 25.9.2025
  ...
  test "has_subtasks? returns true when subtask tag is present" do
    parser = Connector::SubtaskParser.new(@sample_article_body)
    assert parser.has_subtasks?
  end

  test "has_subtasks? returns false when no subtask tag present" do
    parser = Connector::SubtaskParser.new("Regular article without subtasks")
    assert_not parser.has_subtasks?
  end

  test "handles empty article body" do
    parser = Connector::SubtaskParser.new("")
    assert_empty parser.parse_subtasks
    assert_not parser.has_subtasks?
  end

  test "handles nil article body gracefully" do
    parser = Connector::SubtaskParser.new(nil)
    assert_empty parser.parse_subtasks
    assert_not parser.has_subtasks?
  end
  ...
  test "preserves original line content in SubtaskData" do
    article_body = <<~BODY
      [[poduloha]]<br>   <a href="http://vm-home/#user/profile/6" data-mention-user-id="6" rel="nofollow noreferrer noopener" title="http://vm-home/#user/profile/6">Janko Agent</a>   -   Task with extra spaces   -   25.9.2025
    BODY
